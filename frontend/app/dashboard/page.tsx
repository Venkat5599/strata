...
// The dashboard is a set of dedicated sections, one visible at a time,
// selected from the sidebar. Each section is a full panel with its own room —
// nothing is crammed onto a single page.
const SECTIONS = [
  "status", "ledger", "resolver", "activity", "position", "compliance", "contracts",
] as const;
...
function useSection(): Section {
  ...
  useEffect(() => {
    const read = () => {
      ...
      setSection(SECTIONS.includes(h as Section) ? (h as Section) : "status");
    };
    read();
      ...
      {section === "status" && (
        <section className="panel" id="status">
          <div className="panel-head">
            <h2>Live status</h2>
            <span className="panel-note">every row is a live contract read</span>
          </div>
          <SidebarStatus wide />
        </section>
      )}
